...
export default function CircuitPlayer({ session, mode, settings, onClose }) {
  var exercises = (session.program_exercises || []).sort(function(a, b) { return a.order_index - b.order_index }).map(function(pe) {
    // Force minimum duration for circuit exercises
    var fixed = Object.assign({}, pe)
    if (!fixed.rep_min || fixed.rep_min < 10) { fixed.rep_min = 30; fixed.rep_max = 30 }
    if (fixed.rest_seconds == null) fixed.rest_seconds = 10
    return fixed
  })
  ...
  var emomInterval = settings.emom_interval || 1
  var emomRounds = Math.floor(emomMinutes / emomInterval)
  ...
  function startWorkPhase(ei, rd) {
    ...
    if (dur < 10) dur = 30 // minimum 10s, default 30s for circuit
  ...
  var fortimeCap = settings.fortime_cap || 600

  function startForTime() {
    startCountdown(function() {
      startGlobalCountdown(fortimeCap)
  ...
  function forTimeNext() {
    var nextEi = exIdx + 1
    if (nextEi >= exercises.length) {
      // Completed all exercises! Stop timer
      clearAll(); setGlobalRunning(false); setPhase('done'); try{if(navigator.vibrate)navigator.vibrate([300,100,300,100,500])}catch(e){}
    } else {
      setExIdx(nextEi)
    }
  ...
  function runEmomRound(rd) {
    setRound(rd); setExIdx(0); setPhase('work')
    // Timer for the interval (1, 2, or 3 minutes)
    runTimer(emomInterval * 60, function() {
      try{if(navigator.vibrate)navigator.vibrate([200,100,200])}catch(e){}
      if (rd < emomRounds) {
        runEmomRound(rd + 1)
      } else {
        clearAll(); setGlobalRunning(false); setPhase('done')
      ...
      <div style={S.full}>
        <div style={{textAlign:'center',padding:'48px 24px'}}>
          ...
          <div style={{display:'flex',justifyContent:'center',gap:12,marginBottom:20}}>
            ...
            {mode === 'emom' && <div style={{background:'rgba(196,151,58,0.08)',border:'1px solid rgba(196,151,58,0.15)',borderRadius:12,padding:'10px 16px',textAlign:'center'}}><div style={{fontSize:18,fontWeight:700,color:GOLD}}>{emomRounds} tours</div><div style={{fontSize:9,color:'#7a7065',textTransform:'uppercase',letterSpacing:'0.1em'}}>{emomInterval>1?'E'+emomInterval+'MOM':'EMOM'} — {emomMinutes} min (chaque {emomInterval} min)</div></div>}
            {mode === 'fortime' && <div style={{background:'rgba(196,151,58,0.08)',border:'1px solid rgba(196,151,58,0.15)',borderRadius:12,padding:'10px 16px',textAlign:'center'}}><div style={{fontSize:18,fontWeight:700,color:GOLD}}>⏱️ {fmt(fortimeCap)}</div><div style={{fontSize:9,color:'#7a7065',textTransform:'uppercase',letterSpacing:'0.1em'}}>time cap — termine avant !</div></div>}
          </div>
          <div style={{marginBottom:24}}>
            {exercises.map(function(pe, i) {
              ...
              return <div key={i} style={{display:'flex',gap:12,alignItems:'center',padding:'8px 12px',maxWidth:320,margin:'0 auto',background:'rgba(255,255,255,0.02)',borderRadius:10,marginBottom:4}}>
                ...
                {mode === 'circuit' && <div style={{fontSize:10,color:'#555',background:'rgba(255,255,255,0.04)',padding:'2px 8px',borderRadius:6}}>{(pe.rep_min&&pe.rep_min>=10?pe.rep_min:30)}s</div>}
              </div>
            })}
          </div>
    ...
    return (
      <div style={S.full}>
        <div style={{textAlign:'center',padding:'48px 24px'}}>
          ...
          <div style={{fontFamily:'Cormorant Garamond,serif',fontSize:28,marginBottom:16}}>Bravo !</div>
          {mode === 'amrap' && <div>
            <div style={{fontSize:18,color:'#f0ece4',marginBottom:8}}>Tu as réalisé</div>
            <div style={{fontSize:56,fontWeight:800,color:GOLD,marginBottom:4}}>{amrapRounds}</div>
            <div style={{fontSize:18,color:'#f0ece4',marginBottom:16}}>tour{amrapRounds > 1 ? 's' : ''} 💪</div>
            <div style={{fontSize:13,color:'#7a7065',marginBottom:12}}>C'est correct ?</div>
            <div style={{display:'flex',gap:8,justifyContent:'center',marginBottom:20}}>
              <button onClick={function(){setAmrapRounds(function(r){return Math.max(0,r-1)})}} style={{width:44,height:44,borderRadius:12,border:'1px solid rgba(248,113,113,0.3)',background:'rgba(248,113,113,0.08)',color:'#f87171',fontSize:20,fontWeight:700,cursor:'pointer'}}>−</button>
              <div style={{width:60,height:44,borderRadius:12,border:'1px solid rgba(196,151,58,0.3)',background:'rgba(196,151,58,0.08)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:22,fontWeight:800,color:GOLD}}>{amrapRounds}</div>
              <button onClick={function(){setAmrapRounds(function(r){return r+1})}} style={{width:44,height:44,borderRadius:12,border:'1px solid rgba(74,222,128,0.3)',background:'rgba(74,222,128,0.08)',color:'#4ade80',fontSize:20,fontWeight:700,cursor:'pointer'}}>+</button>
            </div>
          </div>}
          {mode === 'fortime' && <div>
            <div style={{fontSize:18,color:'#f0ece4',marginBottom:8}}>Tu as terminé en</div>
            <div style={{fontSize:48,fontWeight:800,color:GOLD,marginBottom:16,fontFamily:'Outfit'}}>{fmt(globalTimer)}</div>
          </div>}
          {mode !== 'amrap' && mode !== 'fortime' && <div style={{fontSize:18,color:GOLD,marginBottom:16}}>Séance terminée !</div>}
          <button onClick={function(){
            if(onClose){onClose(mode === 'amrap' ? { amrapRounds: amrapRounds } : mode === 'fortime' ? { fortimeSeconds: globalTimer } : {})}
          }} style={S.btnG}>✓ Valider et fermer</button>
        </div>
      </div>
    )
  }
    ...
    <div style={{...S.full,background:isRest?'#061a08':'#0a0a0a'}}>
      ...
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'16px 20px',borderBottom:'1px solid rgba(255,255,255,0.06)'}}>
        ...
        <div style={{minWidth:70,textAlign:'right'}}>
          ...
          {mode === 'emom' && <div style={{fontSize:12,color:'#777'}}>Tour {round}/{emomRounds}</div>}
      ...
      <div style={{flex:1,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',padding:'0 24px'}}>
        ...
        {!isRest && mode !== 'emom' && <div style={{fontSize:26,fontWeight:700,marginBottom:4,textAlign:'center',letterSpacing:'-0.02em'}}>{ex && ex.name || 'Exercice'}</div>}

        {/* EMOM: show ALL exercises with reps */}
        {!isRest && mode === 'emom' && (
          <div style={{width:'100%',maxWidth:360}}>
            <div style={{fontSize:12,color:GOLD,textTransform:'uppercase',letterSpacing:'0.15em',textAlign:'center',marginBottom:12,fontWeight:700}}>Tour {round}/{emomRounds} — {emomInterval>1?'E'+emomInterval+'MOM':'EMOM'}</div>
            {exercises.map(function(pe, pi) {
              var exInfo = pe.exercises || pe
              return <div key={pi} style={{display:'flex',alignItems:'center',gap:10,padding:'10px 12px',background:'rgba(255,255,255,0.03)',borderRadius:10,marginBottom:6,border:'1px solid rgba(255,255,255,0.06)'}}>
                {exInfo.gif_url && <img src={exInfo.gif_url} style={{width:40,height:40,borderRadius:8,objectFit:'cover'}}/>}
                <div style={{flex:1}}>
                  <div style={{fontSize:15,fontWeight:600}}>{exInfo.name || '?'}</div>
                  <div style={{fontSize:11,color:'#7a7065'}}>{pe.rep_min ? pe.rep_min + ' reps' : 'Max reps'}</div>
                </div>
                <div style={{fontSize:20,fontWeight:800,color:GOLD}}>{pe.rep_min || '∞'}</div>
              </div>
            })}
          </div>
        )}
        ...
        {(mode === 'circuit' || mode === 'tabata' || mode === 'emom') && (
          <div style={{marginBottom:24}}>
            <div style={{fontSize:88,fontWeight:700,fontFamily:'Outfit',color:isRest?'#4ade80':GOLD,textAlign:'center',textShadow:isRest?'0 0 40px rgba(74,222,128,0.2)':'0 0 40px rgba(196,151,58,0.2)'}}>{timer || (phase==='work' ? '...' : 0)}</div>
        ...
        {/* AMRAP / ForTime - show all exercises + manual advance */}
        {mode === 'amrap' && !isRest && (
          <div style={{width:'100%',maxWidth:360}}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:10}}>
              <div style={{fontSize:12,color:GOLD,textTransform:'uppercase',letterSpacing:'0.15em',fontWeight:700}}>💀 AMRAP</div>
              <div style={{fontSize:28,fontWeight:900,color:GOLD}}>{amrapRounds} tour{amrapRounds>1?'s':''}</div>
            </div>
            {exercises.map(function(pe, pi) {
              var exInfo = pe.exercises || pe
              var isCurrent = pi === exIdx
              return <button key={pi} onClick={function(){
                setExIdx(pi)
              }} style={{display:'flex',alignItems:'center',gap:10,padding:'10px 12px',background:isCurrent?'rgba(196,151,58,0.08)':'rgba(255,255,255,0.02)',borderRadius:10,marginBottom:4,border:isCurrent?'1px solid rgba(196,151,58,0.3)':'1px solid rgba(255,255,255,0.04)',width:'100%',cursor:'pointer',fontFamily:'Outfit',color:'#f0ece4',textAlign:'left'}}>
                {exInfo.gif_url && <img src={exInfo.gif_url} style={{width:36,height:36,borderRadius:8,objectFit:'cover'}}/>}
                <div style={{flex:1}}>
                  <div style={{fontSize:14,fontWeight:isCurrent?700:400}}>{exInfo.name || '?'}</div>
                </div>
                <div style={{fontSize:18,fontWeight:800,color:isCurrent?GOLD:'#555'}}>{pe.rep_min || '∞'}</div>
                {isCurrent && <div style={{fontSize:11,color:GOLD,fontWeight:600}}>▶</div>}
              </button>
            })}
            <button onClick={function(){setAmrapRounds(function(r){return r+1});setExIdx(0)}} style={{width:'100%',marginTop:12,padding:'16px',background:'linear-gradient(135deg,#4ade80,#22c55e)',color:'#000',border:'none',borderRadius:12,fontSize:16,fontWeight:800,cursor:'pointer',fontFamily:'Outfit',boxShadow:'0 4px 15px rgba(74,222,128,0.3)'}}>🔄 Tour complété ! ({amrapRounds+1})</button>
          </div>
        )}

        {mode === 'fortime' && !isRest && (
          <div style={{width:'100%',maxWidth:360}}>
            <div style={{fontSize:48,fontWeight:700,color:GOLD,marginBottom:12,fontFamily:'Outfit',textAlign:'center'}}>{fmt(globalTimer)}</div>
            {exercises.map(function(pe, pi) {
              var exInfo = pe.exercises || pe
              var isCurrent = pi === exIdx
              return <button key={pi} onClick={function(){
