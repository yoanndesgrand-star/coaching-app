...
export default function WorkoutPlayer({ program, profileId, profileName, onClose }) {
  ...
  // Fallback: if session mode is non-normal but no blocks detected, create one
  if (blocks.length === 0 && program.mode && program.mode !== 'normal') {
    blocks = [{ mode: program.mode, settings: program.settings || {}, startIdx: 0, endIdx: programExercises.length - 1, name: '' }]
  }
  // Also check: if all blocks are 'normal' but session mode isn't, fix the first block
  if (blocks.length > 0 && blocks.every(function(b){return b.mode==='normal'}) && program.mode && program.mode !== 'normal') {
    blocks = [{ mode: program.mode, settings: program.settings || {}, startIdx: 0, endIdx: programExercises.length - 1, name: '' }]
  }
  ...
  var pendingTimerCb = useRef(null)
  function startTimer(sec, afterCb) {
    if(timerRef.current) clearInterval(timerRef.current)
    pendingTimerCb.current = afterCb || null
    ...
    timerRef.current = setInterval(function() {
      ...
      if (remaining <= 0) {
        ...
        if (pendingTimerCb.current) { var cb = pendingTimerCb.current; pendingTimerCb.current = null; setTimeout(function(){ cb() }, 500) }
        return
      }
      setTimer(remaining)
    }, 300)
  ...
  var [woQuickCreate, setWoQuickCreate] = useState(false)
  var [woQName, setWoQName] = useState('')
  var [woQMuscle, setWoQMuscle] = useState('')
  var [woQEquip, setWoQEquip] = useState('')
  var [woQGif, setWoQGif] = useState('')
  ...
  return (
    <div style={{...S.full,background:T.bg,color:T.text}}>
      ...
      {circuitBlock && <div style={{position:'fixed',top:0,left:0,right:0,bottom:0,zIndex:200,background:'var(--bg)'}}>
        <CircuitPlayer session={{ program_exercises: exercises.slice(circuitBlock.startIdx, circuitBlock.endIdx + 1).map(function(item) { return Object.assign({}, item.pe, { rep_min: Math.max(item.pe.rep_min || 0, circuitBlock.mode === 'circuit' ? 30 : (item.pe.rep_min || 10)), rest_seconds: item.pe.rest_seconds != null ? item.pe.rest_seconds : 10 }) }) }} mode={circuitBlock.mode} settings={circuitBlock.settings || {}} onClose={function() {
        ...
        {exercises.map(function(item,ei){
          ...
          if (block && block.mode !== 'normal') {
            ...
            return <div key={ei} style={{marginBottom:12,background:blockDone?'rgba(74,222,128,0.05)':'rgba(196,151,58,0.05)',border:'1px solid',borderColor:blockDone?'rgba(74,222,128,0.2)':'rgba(196,151,58,0.2)',borderRadius:16,padding:16,opacity:blockDone?0.7:1}}>
              ...
              {block.mode === 'amrap' && <div style={{fontSize:13,color:T.muted,marginBottom:8}}>💀 Durée : <strong style={{color:GOLD}}>{(function(){var d=block.settings.duration||600;var m=Math.floor(d/60);var s=d%60;return m+'min'+(s>0?' '+s+'s':'')})()}</strong> — Max de tours</div>}
              {block.mode === 'fortime' && <div style={{fontSize:13,color:T.muted,marginBottom:8}}>⏱️ Time Cap : <strong style={{color:GOLD}}>{(function(){var d=block.settings.fortime_cap||600;var m=Math.floor(d/60);var s=d%60;return m+'min'+(s>0?' '+s+'s':'')})()}</strong> — Termine avant !</div>}
              {block.mode === 'emom' && <div style={{fontSize:13,color:T.muted,marginBottom:8}}>⏰ {(block.settings.emom_interval||1)>1?'E'+(block.settings.emom_interval)+'MOM':'EMOM'} : <strong style={{color:GOLD}}>{block.settings.emom_minutes||10} min</strong> — {Math.floor((block.settings.emom_minutes||10)/(block.settings.emom_interval||1))} tours de {block.settings.emom_interval||1} min</div>}
              {block.mode === 'circuit' && <div style={{fontSize:13,color:T.muted,marginBottom:8}}>🔄 {block.settings.rounds||1} tour{(block.settings.rounds||1)>1?'s':''}</div>}
              {blockExercises.map(function(bex,bi) {
                var isCircuitMode = block.mode === 'circuit'
                var dur = bex.pe.rep_min
                if (isCircuitMode && (!dur || dur < 10)) dur = 30
                return <div key={bi} style={{display:'flex',alignItems:'center',gap:8,padding:'6px 0',borderBottom:'1px solid rgba(255,255,255,0.04)'}}>
                  {bex.ex&&bex.ex.gif_url&&<img src={bex.ex.gif_url} style={{width:32,height:32,borderRadius:6,objectFit:'cover'}}/>}
                  <div style={{flex:1,fontSize:13}}>{bex.ex&&bex.ex.name||'?'}</div>
                  {isCircuitMode ? <div style={{display:'flex',alignItems:'center',gap:3}}>
                    <input type="number" inputMode="numeric" value={dur} onChange={function(e){var v=parseInt(e.target.value)||30;setExercises(function(exs){return exs.map(function(x,xi){if(xi!==block.startIdx+bi)return x;var newPe=Object.assign({},x.pe,{rep_min:v,rep_max:v});return Object.assign({},x,{pe:newPe})})})}} style={{width:40,padding:'4px',borderRadius:6,border:'1px solid rgba(196,151,58,0.2)',background:'rgba(196,151,58,0.05)',color:GOLD,fontSize:12,textAlign:'center',fontFamily:'Outfit',outline:'none'}}/>
                    <span style={{fontSize:9,color:T.muted}}>s</span>
                  </div> : <div style={{fontSize:13,fontWeight:700,color:GOLD}}>{bex.pe.rep_min||'—'} <span style={{fontSize:10,fontWeight:400,color:T.muted}}>reps</span></div>}
                </div>
              })}
              ...
              {blockDone && <div style={{textAlign:'center',marginTop:8}}>
                <div style={{fontSize:13,color:'#4ade80',marginBottom:8}}>✅ Terminé</div>
                <button onClick={function(){
                  setExercises(function(exs){return exs.map(function(x,xi){
                    if(xi>=block.startIdx&&xi<=block.endIdx){return Object.assign({},x,{sets:x.sets.map(function(s){return Object.assign({},s,{done:false})})})}
                    return x
                  })})
                  setCircuitBlock(block)
                }} style={{padding:'10px 20px',background:'transparent',border:'1px solid '+GOLD,color:GOLD,borderRadius:10,fontSize:13,fontWeight:600,cursor:'pointer',fontFamily:'Outfit'}}>🔄 Refaire</button>
              </div>}
            </div>
          }
          ...
          return (<div key={ei}>{blockHeader}<div style={{marginBottom:12,background:T.surface,border:'1px solid '+T.border,borderRadius:16,overflow:'hidden',opacity:allDone?0.7:1,transition:'opacity 0.3s'}}>
            ...
            <div style={{display:'flex',alignItems:'center',gap:14,padding:'16px 16px 12px'}}>
              ...
              <div style={{flex:1,minWidth:0}}>
                ...
                {item.pe.rep_mode==='duration'?<div style={{display:'flex',alignItems:'center',gap:6,marginTop:3}}>
                  <button onClick={function(e){e.stopPropagation();
                    var dur=item.pe.rep_min||60
                    var restSec=item.pe.rest_seconds||90
                    startTimer(dur, function(){ startTimer(restSec) })
                  }} style={{background:'rgba(74,222,128,0.1)',border:'1px solid rgba(74,222,128,0.3)',borderRadius:8,padding:'4px 10px',cursor:'pointer',display:'flex',alignItems:'center',gap:4,fontFamily:'Outfit',fontSize:11,color:'#4ade80',fontWeight:600}}>▶ {Math.floor((item.pe.rep_min||60)/60)>0?Math.floor((item.pe.rep_min||60)/60)+'min ':''}{(item.pe.rep_min||60)%60>0?(item.pe.rep_min||60)%60+'s':''}</button>
                  <span style={{fontSize:10,color:T.muted}}>puis {Math.floor((item.pe.rest_seconds||90)/60)>0?Math.floor((item.pe.rest_seconds||90)/60)+'min ':''}{(item.pe.rest_seconds||90)%60}s repos</span>
                </div>:<div style={{fontSize:11,color:T.muted,marginTop:3}}>⏱ {restMin>0?restMin+'min ':''}{restS>0?restS+'s':''} repos {allDone?'· ✅ Terminé':''}</div>}
              </div>
            ...
            {/* Table header - hide for duration mode */}
            {item.pe.rep_mode !== 'duration' && <div>
            ...
            {/* Duration mode: clean display with big play button */}
            {item.pe.rep_mode === 'duration' && <div style={{padding:'16px',textAlign:'center'}}>
              <div style={{fontSize:36,fontWeight:800,color:GOLD,marginBottom:8}}>{Math.floor((item.pe.rep_min||60)/60)>0?Math.floor((item.pe.rep_min||60)/60)+' min ':''}{(item.pe.rep_min||60)%60>0?(item.pe.rep_min||60)%60+'s':''}</div>
              <button onClick={function(){
                var dur=item.pe.rep_min||60
                var restSec=item.pe.rest_seconds||90
                startTimer(dur, function(){ startTimer(restSec) })
              }} style={{padding:'16px 40px',background:'linear-gradient(135deg,#4ade80,#22c55e)',color:'#000',border:'none',borderRadius:14,fontSize:18,fontWeight:800,cursor:'pointer',fontFamily:'Outfit',boxShadow:'0 4px 15px rgba(74,222,128,0.3)'}}>▶ Lancer le chrono</button>
              <div style={{fontSize:12,color:T.muted,marginTop:8}}>puis {Math.floor((item.pe.rest_seconds||90)/60)>0?Math.floor((item.pe.rest_seconds||90)/60)+'min ':''}{ (item.pe.rest_seconds||90)%60}s repos</div>
            </div>}
          </div></div>)
        })}
        ...
        <div style={{position:'fixed',top:0,left:0,right:0,bottom:0,background:'rgba(0,0,0,0.85)',zIndex:20,display:'flex',alignItems:'flex-end',justifyContent:'center'}} onClick={function(e){if(e.target===e.currentTarget){setShowAddExWo(false);setWoMuscle('');setWoEquip('')}}}>
          <div style={{background:'#1a1a1a',borderRadius:'16px 16px 0 0',padding:'20px',width:'100%',maxWidth:480,maxHeight:'70vh',overflow:'auto'}}>
            ...
            <div style={{display:'flex',gap:8,marginTop:8}}>
              <button onClick={function(){setWoQuickCreate(!woQuickCreate)}} style={{flex:1,padding:'12px',background:woQuickCreate?'rgba(196,151,58,0.1)':'none',border:'1px solid rgba(196,151,58,0.3)',borderRadius:8,color:GOLD,fontSize:13,fontWeight:600,cursor:'pointer',fontFamily:'Outfit'}}>{woQuickCreate?'← Retour':'➕ Créer'}</button>
              <button onClick={function(){setShowAddExWo(false);setWoMuscle('');setWoEquip('');setWoQuickCreate(false)}} style={{flex:1,padding:'12px',background:'none',border:'1px solid rgba(255,255,255,0.1)',borderRadius:8,color:'#7a7065',fontSize:13,cursor:'pointer',fontFamily:'Outfit'}}>Fermer</button>
            </div>
            {woQuickCreate && <div style={{marginTop:10,padding:12,background:'rgba(196,151,58,0.04)',border:'1px solid rgba(196,151,58,0.15)',borderRadius:10}}>
              <div style={{fontSize:11,fontWeight:600,color:GOLD,marginBottom:8}}>Créer un exercice</div>
              <input value={woQName} onChange={function(e){setWoQName(e.target.value)}} placeholder="Nom de l'exercice" style={{width:'100%',background:'rgba(255,255,255,0.05)',border:'1px solid rgba(255,255,255,0.1)',borderRadius:8,padding:'10px 12px',color:'#f0ece4',fontSize:13,fontFamily:'Outfit',outline:'none',boxSizing:'border-box',marginBottom:6}}/>
              <div style={{display:'flex',gap:6,marginBottom:6}}>
                <select value={woQMuscle} onChange={function(e){setWoQMuscle(e.target.value)}} style={{flex:1,background:'rgba(255,255,255,0.05)',border:'1px solid rgba(255,255,255,0.1)',borderRadius:6,padding:'8px',color:'#f0ece4',fontSize:11,fontFamily:'Outfit'}}><option value="">Muscle</option>{['Pectoraux','Dos','Épaules','Biceps','Triceps','Abdominaux','Quadriceps','Ischio-jambiers','Mollets','Fessiers','Full Body','Cardio'].map(function(m){return <option key={m} value={m}>{m}</option>})}</select>
                <select value={woQEquip} onChange={function(e){setWoQEquip(e.target.value)}} style={{flex:1,background:'rgba(255,255,255,0.05)',border:'1px solid rgba(255,255,255,0.1)',borderRadius:6,padding:'8px',color:'#f0ece4',fontSize:11,fontFamily:'Outfit'}}><option value="">Équipement</option>{['Barre','Haltères','Machine','Poulie','Poids de corps','Élastique','Kettlebell','TRX','Autre'].map(function(m){return <option key={m} value={m}>{m}</option>})}</select>
              </div>
              <div style={{display:'flex',gap:6,marginBottom:8}}>
                <label style={{flex:1,display:'flex',alignItems:'center',justifyContent:'center',gap:4,padding:'10px',background:'rgba(196,151,58,0.06)',border:'1px dashed rgba(196,151,58,0.3)',borderRadius:8,cursor:'pointer',fontSize:12,color:GOLD,fontFamily:'Outfit'}}>
                  📷 Photo / GIF
                  <input type="file" accept="image/*,.gif" capture="environment" style={{display:'none'}} onChange={async function(e){
                    var file=e.target.files[0]; if(!file)return
                    var fname='exercise-'+Date.now()+'.'+file.name.split('.').pop()
                    await supabase.storage.createBucket('exercises',{public:true}).catch(function(){})
                    var {error}=await supabase.storage.from('exercises').upload(fname,file,{contentType:file.type})
                    if(error){alert('Erreur: '+error.message);return}
                    var {data:u}=supabase.storage.from('exercises').getPublicUrl(fname)
                    if(u&&u.publicUrl)setWoQGif(u.publicUrl)
                  }}/>
                </label>
                {woQGif&&<img src={woQGif} style={{width:48,height:48,borderRadius:8,objectFit:'cover',border:'1px solid rgba(196,151,58,0.3)'}}/>}
              </div>
              <button disabled={!woQName.trim()} onClick={async function(){
                var newEx={name:woQName.trim(),muscle_group:woQMuscle||'Autre',equipment:woQEquip||'Autre',gif_url:woQGif||''}
                var {data}=await supabase.from('exercises').insert(newEx).select().single()
                if(data){addExerciseToWorkout(data);setWoQName('');setWoQGif('');setWoQuickCreate(false)}
              }} style={{width:'100%',padding:'12px',background:GOLD,color:'#000',border:'none',borderRadius:8,fontSize:14,fontWeight:700,cursor:'pointer',fontFamily:'Outfit',opacity:woQName.trim()?1:0.4}}>✓ Créer et ajouter</button>
            </div>}
          </div>
        </div>
      )}
