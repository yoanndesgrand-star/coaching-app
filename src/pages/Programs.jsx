...
export default function Programs({ onBack, coachMode, coachClientId, coachClientName, clients, setCoachClient, coachId, isSuperAdmin, onEditingChange, onLiveTraining, liveTrainingActive, profile }) {
  ...
  var [quickCreate, setQuickCreate] = useState(false)
  var [pickerSnapshot, setPickerSnapshot] = useState(null) // items count when picker opened
  var [quickExName, setQuickExName] = useState('')
  var [quickExMuscle, setQuickExMuscle] = useState('')
  var [quickExEquip, setQuickExEquip] = useState('')
  var [quickExGif, setQuickExGif] = useState('') // null = append at end, number = insert after this index
  ...
  async function saveProgram() {
    ...
    for(var si=0;si<sessions.length;si++){
      ...
      var sr=await supabase.from('program_sessions').insert({program_id:progId,name:sess.name,order_index:si,workout_mode:sess.mode||'normal',mode_settings:JSON.stringify({rounds:sess.rounds||1,tabata_work:sess.tabata_work||20,tabata_rest:sess.tabata_rest||10,tabata_rounds:sess.tabata_rounds||8,duration:sess.duration||600,fortime_cap:sess.fortime_cap||600,emom_minutes:sess.emom_minutes||10,emom_interval:sess.emom_interval||1})}).select().single()
      if(sr.data){
        // Compute block mode for each exercise from separators
        var currentBlockMode = sess.mode || 'normal'
        var currentBlockSettings = {rounds:sess.rounds||1,tabata_work:sess.tabata_work||20,tabata_rest:sess.tabata_rest||10,tabata_rounds:sess.tabata_rounds||8,duration:sess.duration||600,fortime_cap:sess.fortime_cap||600,emom_minutes:sess.emom_minutes||10,emom_interval:sess.emom_interval||1}
  ...
  function editProgram(prog) {
    ...
    setSessions((prog.program_sessions||[]).map(function(s){
      ...
      return{name:s.name,mode:s.workout_mode||'normal',rounds:ms.rounds||1,tabata_work:ms.tabata_work||20,tabata_rest:ms.tabata_rest||10,tabata_rounds:ms.tabata_rounds||8,duration:ms.duration||600,fortime_cap:ms.fortime_cap||600,emom_minutes:ms.emom_minutes||10,emom_interval:ms.emom_interval||1,items:(function(){
        var items = []
        var lastBlockName = ''
  ...
  function addExToSession(ex){setSessions(function(s){return s.map(function(sess,i){if(i!==activeSession)return sess;var newEx={type:'exercise',exercise_id:ex.id,exercise:ex,sets:1,sets_detail:[{t:'work',w:'',r:''}],rep_min:8,rep_max:12,rep_mode:'range',rest_seconds:90,notes:'',superset_group:null};
  ...
  return (
    <div style={{maxWidth:900,margin:'0 auto',padding:'32px 16px'}}>
      ...
      {view==='edit-program'&&<div>
        ...
        {sessions[activeSession]&&<div>
          ...
          {(function(){
            ...
            sess.items.forEach(function(item){
              ...
              if(curMode==='normal'){
                ...
                var reps=item.rep_min||10
                var repTime=Math.ceil(reps*3.5) // ~3.5s per rep
                var restPer=item.rest_seconds||90
                totalSec+=sets*(repTime+restPer)
              } else if(curMode==='circuit'){
                totalSec+=(item.rep_min||30)+(item.rest_seconds||10)
              }
            })
            // For block-level modes, add their configured durations
            sess.items.forEach(function(item){
              if(item.type!=='separator')return
              if(item.mode==='amrap')totalSec+=((item.block_settings||{}).duration||600)
              if(item.mode==='fortime')totalSec+=((item.block_settings||{}).fortime_cap||600)
              if(item.mode==='emom')totalSec+=(((item.block_settings||{}).emom_minutes||10)*60)
              if(item.mode==='tabata'){var bs=item.block_settings||{};var nExAfter=0;var idx=sess.items.indexOf(item);for(var j=idx+1;j<sess.items.length;j++){if(sess.items[j].type==='separator')break;if(sess.items[j].type==='exercise')nExAfter++};totalSec+=nExAfter*(bs.tabata_work||20+bs.tabata_rest||10)*(bs.tabata_rounds||8)}
            })
            // Session-level modes override
            if(sess.mode==='amrap')totalSec=sess.duration||600
            if(sess.mode==='fortime')totalSec=sess.fortime_cap||600
            if(sess.mode==='emom')totalSec=(sess.emom_minutes||10)*60
            if(sess.mode==='tabata'){var nEx=sess.items.filter(function(i){return i.type==='exercise'}).length;totalSec=nEx*((sess.tabata_work||20)+(sess.tabata_rest||10))*(sess.tabata_rounds||8)}
            var min=Math.floor(totalSec/60)
            return <div style={{fontSize:10,color:'var(--muted)',marginBottom:8,display:'flex',alignItems:'center',gap:6}}>⏱️ Durée estimée : <strong style={{color:GOLD}}>{min>0?min+' min':totalSec+'s'}</strong></div>
          })()}
          ...
          <div style={{background:'rgba(196,151,58,0.04)',border:'1px solid rgba(196,151,58,0.15)',borderRadius:10,padding:12,marginBottom:12}}>
            ...
            {sessions[activeSession].mode==='amrap'&&(function(){
              var dur=sessions[activeSession].duration||600
              var mins=Math.floor(dur/60); var secs=dur%60
              return <div style={{display:'flex',gap:8,alignItems:'center',flexWrap:'wrap'}}>
                <div><div style={{fontSize:8,color:'var(--muted)'}}>MINUTES</div><input type="number" inputMode="numeric" value={mins} onChange={function(e){var m=parseInt(e.target.value)||0;setSessions(function(s){return s.map(function(sess,i){if(i!==activeSession)return sess;return Object.assign({},sess,{duration:m*60+secs})})})}} style={{...S.ip,width:65,padding:'8px',fontSize:16,textAlign:'center',fontWeight:700}}/></div>
                <span style={{fontSize:16,color:'var(--muted)',fontWeight:700}}>:</span>
                <div><div style={{fontSize:8,color:'var(--muted)'}}>SECONDES</div><input type="number" inputMode="numeric" value={secs} onChange={function(e){var s=Math.min(59,parseInt(e.target.value)||0);setSessions(function(ss){return ss.map(function(sess,i){if(i!==activeSession)return sess;return Object.assign({},sess,{duration:mins*60+s})})})}} style={{...S.ip,width:65,padding:'8px',fontSize:16,textAlign:'center',fontWeight:700}}/></div>
                <div style={{fontSize:11,color:'var(--muted)',marginLeft:4}}>💀 Max de tours en {mins>0?mins+'min':''}{'  '}{secs>0?secs+'s':''}</div>
              </div>
            })()}
            {sessions[activeSession].mode==='emom'&&(function(){
              var mins=sessions[activeSession].emom_minutes||10
              var interval=sessions[activeSession].emom_interval||1
              var rounds=Math.floor(mins/interval)
              var valid=mins%interval===0
              return <div>
                <div style={{display:'flex',gap:8,alignItems:'center',flexWrap:'wrap',marginBottom:6}}>
                  <div><div style={{fontSize:8,color:'var(--muted)'}}>DURÉE TOTALE</div><div style={{display:'flex',alignItems:'center',gap:4}}><input type="number" value={mins} onChange={function(e){var v=parseInt(e.target.value)||1;setSessions(function(s){return s.map(function(sess,i){if(i!==activeSession)return sess;return Object.assign({},sess,{emom_minutes:v})})})}} style={{...S.ip,width:55,padding:'4px',fontSize:12,textAlign:'center'}}/><span style={{fontSize:10,color:'var(--muted)'}}>min</span></div></div>
                  <div><div style={{fontSize:8,color:'var(--muted)'}}>INTERVALLE</div><select value={interval} onChange={function(e){var v=parseInt(e.target.value)||1;setSessions(function(s){return s.map(function(sess,i){if(i!==activeSession)return sess;return Object.assign({},sess,{emom_interval:v})})})}} style={{...S.ip,width:110,padding:'4px',fontSize:11}}><option value={1}>Chaque 1 min</option><option value={2}>Chaque 2 min</option><option value={3}>Chaque 3 min</option><option value={4}>Chaque 4 min</option><option value={5}>Chaque 5 min</option></select></div>
                  <div><div style={{fontSize:8,color:'var(--muted)'}}>TOURS</div><div style={{fontSize:16,fontWeight:700,color:valid?GOLD:'#f87171'}}>{valid?rounds:'⚠️'}</div></div>
                </div>
                {!valid&&<div style={{fontSize:11,color:'#f87171',padding:'6px 10px',background:'rgba(248,113,113,0.06)',borderRadius:6,marginBottom:4}}>⚠️ {mins} min n'est pas divisible par {interval}. Choisis {Math.floor(mins/interval)*interval} ou {(Math.floor(mins/interval)+1)*interval} minutes.</div>}
                {valid&&<div style={{fontSize:10,color:'var(--muted)',marginBottom:4}}>⏰ {rounds} tour{rounds>1?'s':''} de {interval} min — Réalise tous les exercices ci-dessous à chaque intervalle</div>}
              </div>
            })()}
            <div style={{fontSize:10,color:'#555',marginTop:6,lineHeight:1.5}}>{(function(){var m=sessions[activeSession].mode||'normal';var info={normal:'Séries et répétitions classiques.',circuit:'🔄 Les exercices défilent automatiquement avec les durées indiquées.',tabata:'⚡ Intervalles haute intensité. Travail/repos par rounds.',amrap:'💀 Fais le maximum de tours dans le temps imparti. Appuie sur "Tour ✓" à chaque tour terminé.',fortime:'⏱️ Termine tous les exercices le plus vite possible. Le chrono mesure ton temps.',emom:'⏰ Chaque minute (ou intervalle), réalise les exercices prescrits. EMOM = 1 min, E2MOM = 2 min, etc.'};return info[m]||''})()}</div>
            {sessions[activeSession].mode==='fortime'&&(function(){
              var dur=sessions[activeSession].fortime_cap||600
              var mins=Math.floor(dur/60); var secs=dur%60
              return <div style={{display:'flex',gap:8,alignItems:'center',flexWrap:'wrap',marginTop:6}}>
                <div><div style={{fontSize:8,color:'var(--muted)'}}>TIME CAP (max)</div><div style={{display:'flex',alignItems:'center',gap:4}}><input type="number" inputMode="numeric" value={mins} onChange={function(e){var m=parseInt(e.target.value)||0;setSessions(function(s){return s.map(function(sess,i){if(i!==activeSession)return sess;return Object.assign({},sess,{fortime_cap:m*60+secs})})})}} style={{...S.ip,width:65,padding:'8px',fontSize:16,textAlign:'center',fontWeight:700}}/><span style={{fontSize:16,color:'var(--muted)',fontWeight:700}}>:</span><input type="number" inputMode="numeric" value={secs} onChange={function(e){var s=Math.min(59,parseInt(e.target.value)||0);setSessions(function(ss){return ss.map(function(sess,i){if(i!==activeSession)return sess;return Object.assign({},sess,{fortime_cap:mins*60+s})})})}} style={{...S.ip,width:65,padding:'8px',fontSize:16,textAlign:'center',fontWeight:700}}/></div></div>
                <div style={{fontSize:11,color:'var(--muted)'}}>⏱️ Temps maximum pour finir</div>
              </div>
            })()}
            <button onClick={function(){
              ...
              setInsertAtIdx(lastIdx); setShowAddEx(true);setPickerSnapshot(function(){var counts={};if(sessions[activeSession])sessions[activeSession].items.forEach(function(it){if(it.type==="exercise"&&it.exercise_id)counts[it.exercise_id]=(counts[it.exercise_id]||0)+1});return counts})
              ...
              <div key={ii} style={{background:'rgba(196,151,58,0.04)',border:'1px solid rgba(196,151,58,0.15)',borderRadius:10,padding:12,marginBottom:6}}>
                ...
                <div style={{display:'flex',gap:4,alignItems:'center',flexWrap:'wrap'}}>
                  {[{v:'normal',l:'🏋️ Séries'},{v:'circuit',l:'🔄 Circuit'},{v:'tabata',l:'⚡ Tabata'},{v:'amrap',l:'💀 AMRAP'},{v:'fortime',l:'⏱️ For Time'},{v:'emom',l:'⏰ EMOM'}].map(function(opt){
                    ...
                    return <button key={opt.v} onClick={function(){
                      updateItem(activeSession,ii,'mode',opt.v)
                      // Update exercises in this block with appropriate defaults
                      if(opt.v==='circuit'||opt.v==='tabata'||opt.v==='amrap'||opt.v==='fortime'||opt.v==='emom'){
                        var items=sessions[activeSession].items
                        for(var j=ii+1;j<items.length;j++){
                          if(items[j].type==='separator')break
                          if(items[j].type==='exercise'){
                            if(opt.v==='circuit'&&(items[j].rep_min||0)<10){updateItem(activeSession,j,'rep_min',30);updateItem(activeSession,j,'rep_max',30);updateItem(activeSession,j,'rep_mode','duration');updateItem(activeSession,j,'rest_seconds',10)}
                            if(opt.v==='amrap'||opt.v==='fortime'){updateItem(activeSession,j,'rep_mode','fixed');updateItem(activeSession,j,'rest_seconds',0)}
                          }
                        }
                      }
                    }} style={{background:isActive?GOLD:'var(--surface2)',color:isActive?'#000':'var(--text)',border:isActive?'2px solid '+GOLD:'1px solid var(--border)',borderRadius:8,padding:'5px 10px',fontSize:11,fontWeight:isActive?600:400,cursor:'pointer',fontFamily:'Outfit',transition:'all 0.15s'}}>{opt.l}</button>
                  })}
                  ...
                  {item.mode==='amrap'&&(function(){
                    var bs=item.block_settings||{}; var dur=(bs.duration||600); var mins=Math.floor(dur/60); var secs=dur%60
                    return <div style={{display:'flex',gap:4,alignItems:'center'}}>
                      <input type="number" value={mins} onChange={function(e){var m=parseInt(e.target.value)||0;updateItem(activeSession,ii,'block_settings',Object.assign({},bs,{duration:m*60+secs}))}} style={{...S.ip,width:45,padding:'6px',fontSize:14,textAlign:'center',fontWeight:700}}/>
                      <span style={{fontSize:12,color:'var(--muted)'}}>:</span>
                      <input type="number" value={secs} onChange={function(e){var s=Math.min(59,parseInt(e.target.value)||0);updateItem(activeSession,ii,'block_settings',Object.assign({},bs,{duration:mins*60+s}))}} style={{...S.ip,width:45,padding:'6px',fontSize:14,textAlign:'center',fontWeight:700}}/>
                    </div>
                  })()}
                  {item.mode==='fortime'&&(function(){
                    var bs=item.block_settings||{}; var dur=(bs.fortime_cap||600); var mins=Math.floor(dur/60); var secs=dur%60
                    return <div style={{display:'flex',gap:4,alignItems:'center'}}>
                      <span style={{fontSize:9,color:'#555'}}>Cap</span>
                      <input type="number" value={mins} onChange={function(e){var m=parseInt(e.target.value)||0;updateItem(activeSession,ii,'block_settings',Object.assign({},bs,{fortime_cap:m*60+secs}))}} style={{...S.ip,width:45,padding:'6px',fontSize:14,textAlign:'center',fontWeight:700}}/>
                      <span style={{fontSize:12,color:'var(--muted)'}}>:</span>
                      <input type="number" value={secs} onChange={function(e){var s=Math.min(59,parseInt(e.target.value)||0);updateItem(activeSession,ii,'block_settings',Object.assign({},bs,{fortime_cap:mins*60+s}))}} style={{...S.ip,width:45,padding:'6px',fontSize:14,textAlign:'center',fontWeight:700}}/>
                    </div>
                  })()}
                  {item.mode==='emom'&&(function(){
                    var bs=item.block_settings||{}; var mins=bs.emom_minutes||10; var interval=bs.emom_interval||1; var rounds=Math.floor(mins/interval); var valid=mins%interval===0
                    return <div>
                      <div style={{display:'flex',gap:4,alignItems:'center',flexWrap:'wrap'}}>
                        <input type="number" value={mins} onChange={function(e){updateItem(activeSession,ii,'block_settings',Object.assign({},bs,{emom_minutes:parseInt(e.target.value)||1}))}} style={{...S.ip,width:40,padding:'3px',fontSize:10,textAlign:'center'}}/>
                        <span style={{fontSize:8,color:'#555'}}>min</span>
                        <select value={interval} onChange={function(e){updateItem(activeSession,ii,'block_settings',Object.assign({},bs,{emom_interval:parseInt(e.target.value)||1}))}} style={{...S.ip,width:85,padding:'3px',fontSize:9}}><option value={1}>/ 1 min</option><option value={2}>/ 2 min</option><option value={3}>/ 3 min</option></select>
                        <span style={{fontSize:10,fontWeight:700,color:valid?GOLD:'#f87171'}}>= {valid?rounds+'t':'⚠️'}</span>
                      </div>
                      {!valid&&<div style={{fontSize:9,color:'#f87171',marginTop:2}}>{mins} n'est pas divisible par {interval}</div>}
                    </div>
                  })()}
                </div>
            ...
            return <div key={ii} style={{background:'var(--surface)',border:'1px solid var(--border)',borderRadius:nextSS?'10px 10px 0 0':prevSS&&nextSS?'0':'0 0 10px 10px',borderRadiusX:ssGroup?(isFirstSS?'10px 10px 0 0':isLastSS?'0 0 10px 10px':'0'):10,borderTopLeftRadius:!ssGroup||isFirstSS?10:0,borderTopRightRadius:!ssGroup||isFirstSS?10:0,borderBottomLeftRadius:!ssGroup||isLastSS?10:0,borderBottomRightRadius:!ssGroup||isLastSS?10:0,padding:12,marginBottom:nextSS?0:6,borderLeft:ssGroup?'3px solid '+GOLD:'none',borderTop:prevSS?'1px dashed rgba(196,151,58,0.2)':'1px solid var(--border)'}}>
              ...
              <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:6}}>
                ...
                <div style={{display:'flex',gap:3}}>
                  ...
                  }} title="Superset" style={{...S.sm,color:ssGroup?GOLD:'var(--muted)',fontSize:10,fontWeight:ssGroup?700:400,display:curBM==='normal'?'inline-block':'none'}}>SS</button>
              ...
              {curBM === 'circuit' ? (
                ...
                <div style={{display:'flex',gap:8,marginBottom:6,alignItems:'center',flexWrap:'wrap'}}>
                  <span style={{fontSize:11,fontWeight:600,color:GOLD,padding:'5px 10px',background:'rgba(196,151,58,0.1)',borderRadius:6}}>{curBM === 'amrap' ? '💀' : '⏱️'}</span>
                  <input type="number" inputMode="numeric" value={pe.rep_min||''} onChange={function(e){var v=parseInt(e.target.value)||0;updateItem(activeSession,ii,'rep_min',v);updateItem(activeSession,ii,'rep_max',v);updateItem(activeSession,ii,'rep_mode','fixed')}} placeholder="reps" style={{...S.ip,width:65,padding:'8px',fontSize:16,textAlign:'center',fontWeight:700}}/>
                  <span style={{fontSize:11,color:'var(--muted)'}}>reps</span>
                </div>
              ) : curBM === 'emom' ? (
                <div style={{display:'flex',gap:6,marginBottom:6,alignItems:'center',flexWrap:'wrap'}}>
                  <span style={{fontSize:10,fontWeight:600,color:GOLD,padding:'4px 8px',background:'rgba(196,151,58,0.1)',borderRadius:6}}>⏰</span>
                  <input type="number" inputMode="numeric" value={pe.rep_min||''} onChange={function(e){var v=parseInt(e.target.value)||0;updateItem(activeSession,ii,'rep_min',v);updateItem(activeSession,ii,'rep_max',v);updateItem(activeSession,ii,'rep_mode','fixed')}} placeholder="reps" style={{...S.ip,width:50,padding:'4px',fontSize:12,textAlign:'center',fontWeight:600}}/>
                  <span style={{fontSize:10,color:'var(--muted)'}}>reps</span>
                  <span style={{fontSize:9,color:'#555'}}>à chaque intervalle</span>
                </div>
              ) : (
              <div style={{display:'flex',gap:6,marginBottom:6,alignItems:'center',flexWrap:'wrap'}}>
              ...
              <div style={{display:'flex',gap:6,marginTop:6}}>
                <button onClick={function(){var last=setsDetail.length>0?setsDetail[setsDetail.length-1]:{};var newSD=setsDetail.concat([{t:'work',w:last.w||'',r:last.r||''}]);updateItem(activeSession,ii,'sets_detail',newSD);updateItem(activeSession,ii,'sets',newSD.length)}} style={{...S.bk,border:'1px dashed var(--border)',borderRadius:6,padding:'5px 12px',fontSize:11}}>+ Série</button>
          ...
          <div style={{display:'flex',gap:8,marginBottom:12,flexWrap:'wrap'}}><button onClick={function(){setShowAddEx(true);setPickerSnapshot(function(){var counts={};if(sessions[activeSession])sessions[activeSession].items.forEach(function(it){if(it.type==="exercise"&&it.exercise_id)counts[it.exercise_id]=(counts[it.exercise_id]||0)+1});return counts})}} style={{...S.bt,flex:1}}>+ Exercice</button><button onClick={addSeparator} style={{...S.bk,border:'1px dashed var(--border)',padding:'8px 14px',borderRadius:8}}>+ Bloc</button><button onClick={function(){setShowInsertBlock(true)}} style={{...S.bk,border:'1px dashed var(--border)',padding:'8px 14px',borderRadius:8}}>📦 Insérer</button></div>
          ...
          {showAddEx&&<div style={{position:'fixed',top:0,left:0,right:0,bottom:0,background:'var(--bg)',zIndex:100,display:'flex',flexDirection:'column',padding:0}}>
            ...
            <div style={{maxHeight:250,overflow:'auto'}}>{exercises.filter(function(e){
              ...
              return true
            }).map(function(ex){var totalCount=sessions[activeSession]?sessions[activeSession].items.filter(function(it){return it.type==='exercise'&&it.exercise_id===ex.id}).length:0;var snapCount=pickerSnapshot?pickerSnapshot[ex.id]||0:0;var addedCount=totalCount-snapCount;return <div key={ex.id} style={{display:'flex',alignItems:'center',gap:6,width:'100%',padding:'6px 8px',background:addedCount>0?'rgba(74,222,128,0.06)':'transparent',border:addedCount>0?'1px solid rgba(74,222,128,0.2)':'1px solid var(--border)',borderRadius:5,marginBottom:2,fontSize:11}}>
              {ex.gif_url&&<img src={ex.gif_url} onClick={function(){setZoomImg(ex.gif_url)}} style={{width:36,height:36,borderRadius:4,objectFit:'cover',cursor:'zoom-in'}}/>}
              <div style={{flex:1,fontFamily:'Outfit',color:'var(--text)'}}>{ex.name}{addedCount>0&&<span style={{marginLeft:6,fontSize:9,color:'#4ade80',fontWeight:700}}>✓ ×{addedCount}</span>}</div>
              <span style={{fontSize:9,color:GOLD,marginRight:4}}>{ex.muscle_group}</span>
              {addedCount>0&&<button onClick={function(){setSessions(function(s){return s.map(function(sess,i){if(i!==activeSession)return sess;var lastIdx=-1;sess.items.forEach(function(it,idx){if(it.type==='exercise'&&it.exercise_id===ex.id)lastIdx=idx});if(lastIdx>=0){var items=sess.items.slice();items.splice(lastIdx,1);return Object.assign({},sess,{items:items})}return sess})})}} style={{width:28,height:28,borderRadius:6,border:'1px solid rgba(248,113,113,0.3)',background:'rgba(248,113,113,0.1)',color:'#f87171',fontSize:16,fontWeight:700,cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',padding:0,lineHeight:1}}>−</button>}
              <button onClick={function(){addExToSession(ex)}} style={{width:28,height:28,borderRadius:6,border:'1px solid rgba(74,222,128,0.3)',background:'rgba(74,222,128,0.1)',color:'#4ade80',fontSize:16,fontWeight:700,cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',padding:0,lineHeight:1}}>+</button>
            </div>})}</div>
            <div style={{display:'flex',gap:8}}>
              <button onClick={function(){setQuickCreate(!quickCreate)}} style={{...S.bt,fontSize:11,padding:'6px 12px',background:quickCreate?'#2a2520':''}}>{quickCreate?'← Retour':'➕ Créer'}</button>
              <button onClick={function(){setShowAddEx(false);setProgExMuscle('');setProgExEquip('');setQuickCreate(false)}} style={S.bk}>Fermer</button>
            </div>
            {quickCreate && <div style={{marginTop:10,padding:12,background:'rgba(196,151,58,0.04)',border:'1px solid rgba(196,151,58,0.15)',borderRadius:10}}>
              <div style={{fontSize:11,fontWeight:600,color:GOLD,marginBottom:8}}>Créer un exercice rapidement</div>
              <input value={quickExName} onChange={function(e){setQuickExName(e.target.value)}} placeholder="Nom de l'exercice" style={{...S.ip,marginBottom:6,fontSize:12}}/>
              <div style={{display:'flex',gap:6,marginBottom:6}}>
                <select value={quickExMuscle} onChange={function(e){setQuickExMuscle(e.target.value)}} style={{...S.ip,flex:1,fontSize:11,padding:'6px'}}>
                  <option value="">Muscle</option>
                  {['Pectoraux','Dos','Épaules','Biceps','Triceps','Abdominaux','Quadriceps','Ischio-jambiers','Mollets','Fessiers','Full Body','Cardio'].map(function(m){return <option key={m} value={m}>{m}</option>})}
                </select>
                <select value={quickExEquip} onChange={function(e){setQuickExEquip(e.target.value)}} style={{...S.ip,flex:1,fontSize:11,padding:'6px'}}>
                  <option value="">Équipement</option>
                  {['Barre','Haltères','Machine','Poulie','Poids de corps','Élastique','Kettlebell','TRX','Swiss Ball','Autre'].map(function(m){return <option key={m} value={m}>{m}</option>})}
                </select>
              </div>
              <input value={quickExGif} onChange={function(e){setQuickExGif(e.target.value)}} placeholder="URL du GIF (optionnel)" style={{...S.ip,marginBottom:6,fontSize:11}}/>
              <div style={{display:'flex',gap:6,marginBottom:8}}>
                <label style={{flex:1,display:'flex',alignItems:'center',justifyContent:'center',gap:4,padding:'8px',background:'rgba(196,151,58,0.06)',border:'1px dashed rgba(196,151,58,0.3)',borderRadius:8,cursor:'pointer',fontSize:11,color:GOLD,fontFamily:'Outfit'}}>
                  📷 Photo / GIF
                  <input type="file" accept="image/*,.gif" capture="environment" style={{display:'none'}} onChange={async function(e){
                    var file=e.target.files[0]; if(!file)return
                    var ext=file.name.split('.').pop().toLowerCase()
                    var fname='exercise-'+Date.now()+'.'+ext
                    var {data,error}=await supabase.storage.from('exercises').upload(fname,file,{contentType:file.type})
                    if(error){
                      // Bucket might not exist, try creating
                      await supabase.storage.createBucket('exercises',{public:true})
                      var r2=await supabase.storage.from('exercises').upload(fname,file,{contentType:file.type})
                      if(r2.error){alert('Erreur upload: '+r2.error.message);return}
                    }
                    var {data:urlData}=supabase.storage.from('exercises').getPublicUrl(fname)
                    if(urlData&&urlData.publicUrl)setQuickExGif(urlData.publicUrl)
                  }}/>
                </label>
                {quickExGif&&<img src={quickExGif} style={{width:44,height:44,borderRadius:8,objectFit:'cover',border:'1px solid rgba(196,151,58,0.3)'}}/>}
              </div>
              <button disabled={!quickExName.trim()} onClick={async function(){
                var newEx = {name:quickExName.trim(),muscle_group:quickExMuscle||'Autre',equipment:quickExEquip||'Autre',gif_url:quickExGif||'',coach_id:coachId||null}
                var {data}=await supabase.from('exercises').insert(newEx).select().single()
                if(data){addExToSession(data);setQuickExName('');setQuickExGif('');loadAll()}
              }} style={{...S.bt,width:'100%',opacity:quickExName.trim()?1:0.4}}>✓ Créer et ajouter</button>
            </div>}
          </div></div>}
        </div>}
        ...
        <div style={{display:'flex',gap:10,marginTop:14,flexWrap:'wrap'}}>
          ...
          {sessions.length > 0 && sessions[activeSession] && sessions[activeSession].items.filter(function(it){return it.type==='exercise'}).length > 0 && (
            <button onClick={function(){
              ...
              var exBlocks = []
              var sess = sessions[activeSession]
              var curMode = sess.mode || 'normal'
              var curName = ''; 
              var curSettings = {rounds:sess.rounds||1,tabata_work:sess.tabata_work||20,tabata_rest:sess.tabata_rest||10,tabata_rounds:sess.tabata_rounds||8,duration:sess.duration||600,fortime_cap:sess.fortime_cap||600,emom_minutes:sess.emom_minutes||10,emom_interval:sess.emom_interval||1}
              ...
              setLiveWorkout({programName:pName+' — '+sName,exercises:peList,blocks:blocks,clientId:liveClient||null,mode:sess.mode||'normal',settings:{rounds:sess.rounds||1,tabata_work:sess.tabata_work||20,tabata_rest:sess.tabata_rest||10,tabata_rounds:sess.tabata_rounds||8,duration:sess.duration||600,fortime_cap:sess.fortime_cap||600,emom_minutes:sess.emom_minutes||10,emom_interval:sess.emom_interval||1}})
      ...
      {liveWorkout && (
        <WorkoutPlayer
          program={{ id: null, name: liveWorkout.programName, program_exercises: liveWorkout.exercises, blocks: liveWorkout.blocks || [], mode: liveWorkout.mode || 'normal', settings: liveWorkout.settings || {} }}
